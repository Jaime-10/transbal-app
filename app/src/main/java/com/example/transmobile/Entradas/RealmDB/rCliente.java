...
public class rCliente {
    ...
    public final static mCliente getCliente(final mCliente cliente) {
        Realm realm = Realm.getDefaultInstance();
        mCliente client = realm.where(mCliente.class).equalTo("id_clinete", cliente.getId_cliente()).findFirst();
        return client;
    }

    public final static mCliente getClienteById(final String cliente) {
        Realm realm = Realm.getDefaultInstance();
        mCliente client = realm.where(mCliente.class).equalTo("id_clinete", cliente).findFirst();
        return client;
    }
